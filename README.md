...
## 1 Overview  
This document explains stpes involved installation of Fedora 43 on a UEFI-based x86_64 system
(typical PC/laptop). It involves a fresh installation (no upgrade) and includes steps of boot media
creation and installation.

## 2 Audience & Scope  
### 2.1 Who this is for  
This guide is intended for system administrations or uses familiar with basic system hardware
...
BIOS) and whether OS uses Secure Boot.  
...
3. (Optional) Using the sha256sum or equivalent, compute the checksum and compare the
published one.  

### 4.2 Writing the ISO to USB  
1. Insert the USB stick (8 GB or larger).  
2. Open your chosen tool (Fedora Media Writer, Rufus, Etcher)  
3. Select the downloaded ISO and choose the USB device as target.  
4. Start the write process and then eject the drive once the process is completed.  

### 4.3 Verifying bootability  
1. Reboot your system and enter firmware (BIOS/UEFI) boot menu (typically by pressing
F2/F10/Esc/F12).  
...
2. It will consist of three options. These options will help in determining that whether you
...
### 6.4 Confirmation  
1. This page will give you the recap of the options that you have selected in the installation process.  
...
## 7 First‑Boot Setup  
### 7.1 Welcome  
You can change the language for the system.  
